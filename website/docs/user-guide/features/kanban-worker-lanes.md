...
- **Worker-spawned subprocess self-registering as a new run (fixed)** — a worker's `HERMES_KANBAN_*` env vars (the table above) are process-global, so a worker that shells out to its own `hermes` subprocess via the `terminal` tool — e.g. probing a tool outside its own profile's toolset with `hermes chat -t <tool> -q ...` — used to have that subprocess inherit `HERMES_KANBAN_TASK` / `HERMES_KANBAN_RUN_ID` / `HERMES_KANBAN_BOARD` and self-register as a *second* run against the SAME task id. The nested process only had the restricted `kanban_*`/`tool_search` schema, had no filesystem/terminal tools, and posted spurious `kanban_block` / comments under the real card — burning the task's block-loop budget and occasionally tripping `block_loop_detected` on otherwise-healthy work (see t_1b6aa6fc, runs 223–224). Fixed by stripping the run-identity subset of `HERMES_KANBAN_*` (`TASK`/`RUN_ID`/`CLAIM_LOCK`/`BOARD`/`DB`) from every subprocess the terminal tool spawns, unconditionally — not just `delegate_task` children (`agent/delegation_context.py`'s `KANBAN_IDENTITY_ENV_KEYS` / `scrub_kanban_identity_env`, wired into `tools/environments/local.py`'s `_scrub_delegated_child_kanban_env`). `HERMES_KANBAN_WORKSPACE` is intentionally preserved on this path — it's the directory a worker is expected to `cd` into for its own terminal calls, not run identity. If you need a helper subprocess to see the parent's kanban identity on purpose (rare — a supervisor script that itself drives `hermes kanban` commands against the SAME task), pass those vars explicitly via the command's own `env=` rather than relying on inheritance.
